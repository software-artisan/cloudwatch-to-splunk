...
def process_one_log_stream(client, ner, group_name, stream_name, first_event_time, last_event_time,
    ...
    while (True):
        ...
        for idx, event in enumerate(events):
            ...
            # add every word of msg
            msg_split = msg.split()
            for mp in msg_split:
                add_log_line(tm, msg, mp, all_messages, group_name, stream_name, region)
            # if possible, extract resource name for lambda logs
            if group_name.startswith('/aws/lambda') and 'resourcePath' in msg:
                rp = extract_path(msg)
                if rp:
                    print(f"Found resourcePath {rp}. Adding {msg} to index")
                    add_log_line(tm, msg, rp, all_messages, group_name, stream_name, region)
